...
# 规则:
#   - 检查 packages/entry/host/Bootstrap.cs 中 CordisContext 构造是否传入 logSink
...
if ($MyInvocation.InvocationName -ne '.') {
    ...
    $bootstrap = Join-Path (Get-RepoRoot) 'packages\entry\host\Bootstrap.cs'
    if (-not (Test-Path $bootstrap)) {
        Write-GateFail 'host-log-sink' @('packages/entry/host/Bootstrap.cs — 引导文件不存在')
    }
    $content = Get-Content $bootstrap -Raw
    if (-not (Test-ContentHasLogSink $content)) {
        Write-GateFail 'host-log-sink' @('packages/entry/host/Bootstrap.cs — CordisContext 未挂日志 sink，插件加载失败会被静默吞掉，无法定位运行期故障')
    }
