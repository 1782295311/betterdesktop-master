...
**做什么**：统一桌面环境所有表面的右键菜单——桌面空白处、任务栏、Dock 项、窗口标题栏扩展、以及第三方插件贡献的菜单项。 **不做什么**：
...
- `context-menu.ini`：`shell.integration = native | unified`（默认 `unified`）。 （~~`shell.displayMode = full | grouped`~~ 已于 2026-09-03 删除，见 §模式 说明。）
...
- unified 模式会丢失部分第三方 Shell 扩展项（native 模式保留注册表扩展）。
- COM 枚举依赖系统 Shell，权限/句柄失效/超时场景回退纯插件菜单，不白屏不崩。
- MenuHost 载体（WPF Popup / WebView2 overlay）仍在评估中。
