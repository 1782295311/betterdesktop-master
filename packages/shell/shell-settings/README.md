...
- 内置**设置主分区**（`Sections/SystemSection.cs`，Title="设置"）：由原"通用"与"系统管理"合并而成，含 5 组：常规与外观 / 启动与自启 / 性能与资源 / 维护与重置 / 组件管理。
...
- ✅ **启动即打开设置窗口**（由 `SettingsPlugin.LoadAsync` 直接 `Show()`，与应用提取器的启动行为一致；后续可改为配置开关/菜单入口）。
...
- ✅ **主题分区（导航栏新板块 `ThemeSection`，Title="主题"）**：统一管控程序一切外观，含 5 组真实可调项——①强调色（8 色预设色板，点击即应用）；②窗体透明度与毛玻璃（不透明度滑块 + Acrylic/Transparent 材质开关）；③字号缩放（小/中/大）；④圆角与间距（圆角半径 + 间距缩放滑块）；⑤皮肤（选择本地图片作窗口背景，支持清除）。所有控件经 Mac* 样式且文字亮色；改动经 `IAppearanceService` 实时套用并广播。
...
- 外观颜色经 `IThemeTokens` 集中管理（跟随统一窗口基类毛玻璃材质，不实现深/浅色模式），待 ThemeCenter 落地后可由其实现替换。
