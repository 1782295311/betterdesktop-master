...
├── MenuBarLeftZone / LogoMenuWindow   # 左区：Logo 图标菜单（关于/设置/应用提取器）+ 复用 IStartMenuService


├── MenuBarStatusStrip            # 右区状态条（15 个自绘图标 + ToggleSwitch，内聚单类）


└── Services/StatusBarMenuBarExtension  # 右区唯一 IMenuBarExtension 扩展（经 MenuBarExtensionRegistry 装配）
...
依赖（csproj 实际）：`BetterDesktop.Kernel`、`BetterDesktop.Api`、`shell-core`（ShellWindow/PopupWindowBase/Vibrancy/WinEventPump）、`shell-status`（监控 + 原生 DLL）、`shell-settings`（IAppearanceService 令牌）、`shell-start-menu`、`shell-search`、`shell-app-source`、`shell-window-tracker`、`shell-desktop`、`shell-calendar`、`shell-pinning`。主题令牌真相源 = `IAppearanceService` + `host/App.xaml` 令牌全集（无 theme-center 包，仅设计提案）。弹窗约 20 个全部继承 `PopupWindowBase` 统一弹窗基类。
...

{
    ...
    string Id { get; }                    // 扩展标识（契约已上提 packages/api/Core/IMenuBarExtension.cs）


    FrameworkElement? GetVisual();        // 自绘控件（非按钮扩展返回 null，仅弹层语义）


    void OpenPopup(FrameworkElement anchor);   // 打开扩展自己的弹层


    void ClosePopup();                    // 幂等收起
...

{
    ...
    public IReadOnlyList<Type> Inject => new[] { typeof(IPinningService) };   // 硬依赖：缺失则本插件停在 Pending（不再靠可空 Get 依赖装配顺序）
...

    {
        ...
        // 右区实际为单一内聚扩展（15 个图标含音量/麦克风/蓝牙/WiFi/电量/CPU/内存/亮度/IME/日期时间…），
        // 经 MenuBarExtensionRegistry 装配 3 个扩展：StatusBar / Logo / StacksPopup。
        _statusBar = new StatusBarMenuBarExtension(vol, mic, /* ... 监控服务经 Get 可选注入 ... */);
        context.Get<IMenuBarExtensionRegistry>()?.Register(_statusBar);
        context.Get<IMenuBarExtensionRegistry>()?.Register(new LogoMenuBarExtension(/* ... */));
        context.Get<IMenuBarExtensionRegistry>()?.Register(new StacksPopupBarExtension());
...
- 左区：`MenuBarLeftZone` + `LogoMenuWindow`（Logo 子菜单：关于/设置/应用提取器——后者经 IEventBus `shell.appgrabber.show` 跨包调起 shell-dock）；程序入口复用 `IStartMenuService`（数据来自 `IAppSourceService`）。


- 右区：`IMenuBarExtension` 契约四成员（Id/GetVisual/OpenPopup/ClosePopup，见 packages/api/Core）；右区 15 个图标内聚于单一 `StatusBarMenuBarExtension`。


- 通知按钮：**本地面板**（`NotificationCenterWindow`），系统通知源（WinRT UserNotificationListener）待接入——"调起 Windows 原生通知中心"为旧稿描述，勿采信。
...
Win 键 / Logo 图标 → 开始菜单（复用 shell-start-menu）/ Logo 菜单弹出 → 命令执行
...
- 窗口继承 `ShellWindow`（本体）/ `PopupWindowBase`（弹窗）；外观令牌来自 `IAppearanceService`（shell-settings）；屏幕/DPI 用 `IDesktopSurface`。


- 程序菜单数据/图标经 `IAppSourceService` / `IAppIconService`（shell-app-source）。


- 弹层自管（`MenuBarPopupWindow` 体系）——中央 IMenuService 管线已于 2026-09-05 退役，勿引用；Logo 菜单经 IEventBus `shell.appgrabber.show` 跨包打开应用提取器（shell-dock 订阅）。
...
- [x] 左区：程序菜单入口（`◈`，复用 `IStartMenuService` 打开开始菜单）+ 位置 / 下载 / 文档（交 explorer 打开） —— 注意：这里是**导航入口**已通，左区自有的 CairoMenu（20 项命令）仍属第 14 节开放问题，未实现。
...
- ~~左半区尚未实现~~ **已实现**：`MenuBarLeftZone` 提供程序菜单入口（复用 `IStartMenuService`）+ 位置/下载/文档。 服务未注入时该入口自动不呈现（M10 降级）。左区自有的 CairoMenu 20 项命令仍未实现（见第 14 节）。
...
- 菜单栏本体**仍只驻留主屏**（每显示器一个菜单栏未实现）。但**弹窗锚定已按锚点所在显示器**回钳， 并修正了物理像素/逻辑单位混用（原实现把 `PointToScreen` 的物理点直接当逻辑点用，非 100% DPI 下弹窗会偏移）。
...
- **前景色单一真相源 `MenuBarTheme`**：右区是纯代码自绘（`Rectangle`/`Path`/`TextBlock` 直接赋 `Fill`/`Stroke`）， 历史上有 55 处硬编码 `Brushes.White`，亮色主题下整条菜单栏不可读。 改法不是把 30+ 个对象初始化器拆成 `SetResourceReference` 三段式，而是让 `MenuBarTheme` 持有**一个未冻结的 `SolidColorBrush` 共享实例**——WPF 中未冻结画刷改 `Color` 会自动通知所有引用者重绘， 于是"改一次 = 全菜单栏换色"，效果等同 DynamicResource，但对调用方零结构改动。 保留的 3 处语义白：CapsuleSwitch 滑块、ToggleSwitch 圆钮、静音红底白字（这些是控件自身配色，不随主题前景走）。

- **屏幕几何统一在 `MenuBarScreen`**：所有对外 API 一律**逻辑单位**（与 `Window.Left/Top` 同域）。 `Visual.PointToScreen()` 给物理像素，`SystemParameters.WorkArea` 给逻辑单位，二者不能直接运算。

- **弹窗互斥**：`StatusBarMenuBarExtension.ShowPopup` 打开新面板前先 `CloseAllExcept(popup)`， 仿 macOS 同一时刻只开一个面板（原实现点了 CPU 再点声音会叠两层窗口）。

- **显示器变化自适应**：`MenuBarWindow` 挂 `HwndSource` 钩子监听 `WM_DISPLAYCHANGE` / `WM_SETTINGCHANGE`， 分辨率/缩放/任务栏位置变化后自动重排并收起已开面板（旧锚点已失效）。
...
用户实跑后提出的 7 个问题，已全部修复并通过全量构建门禁（0 警告 0 错误）。本节记录**根因与结论**， 避免后续重犯同类错误。
...
- **图标自绘**：新建 `Contracts/ControlCenterGlyph.cs`（24×24 栅格，11 个图标 + 4 个媒体控制几何）， 取代全部 Segoe MDL2 Assets 码位。Wi‑Fi 直接复用 `WifiGlyph`，保证**菜单栏 / NETWORK 面板 / 控制中心三处一致**。

- **信息对齐**：三张模块卡片共用 `BuildModuleCard`（头部「标题左 / 数值右」+ 内容），卡片内控件垂直居中； 大瓦片图标由 `VerticalAlignment.Top` 改为 `Center`（此前图标顶挂、文字居中 → 视觉错位）。

- **布局升级**：开关网格行高 58 → 60；瓦片/卡片描边由硬编码 `Color.FromArgb(120/60,255,255,255)` 改走主题令牌 `ThemeSeparator`（亮色模式下原描边几乎看不见）。

- **功能对接**：

- 麦克风按钮 → `AudioCoreNative.SetCaptureVolume(vol, !muted)`（保持音量、只翻静音位）， 橙底=已静音，直接读 `GetStatus(AudioFlow.Capture)` 而非等监控轮询。

- 媒体区 → 新建 `Services/MediaSessionController.cs`（SMTC 薄壳，2s 轮询 + 命令后即时回读）， 显示真实 Title/Artist/AppName，Prev / Play-Pause / Next 下发真实媒体命令； 播放状态切换只改同一个 `Path` 的 `Data`，不重建按钮（避免闪烁与丢失悬停态）。

- `ControlCenterFeatureCatalog.ShowAt` 修正单位纪律违例：物理锚点经 `MenuBarScreen.ToLogical` 换算，钳制边界改取**锚点所在显示器**的 `GetWorkArea`（原用 `SystemParameters.WorkArea`，只描述主屏）。
...
1. **字体码位不可信**：Segoe MDL2 Assets 的码位凭印象填 → 字形与语义不符、字体缺失显示方块。 凡是"图标 + 语义"的场景，一律**自绘几何**（`WifiGlyph` / `PowerGlyph` / `ControlCenterGlyph`）。

2. **有入口必须有行为**：纯 `TextBlock` + 无点击事件 = 看起来能点、点了没反应。 所有"偏好设置/跳转"统一走 `NativePanelStyles.CreateSettingsLink`， 所有"看起来是按钮"的元素统一挂点击处理 + `Cursors.Hand`。
...
`MediaPlaybackState` 是**本项目自定义枚举**（`shell-status/Native/MediaCoreNative.cs`）， **不是** WinRT 的 `Windows.Media.Control.MediaPlaybackState`。 且在 `BetterDesktop.Shell.MenuBar.Windows` 命名空间下，裸写 `Windows.Media.Control.X` 会被就近解析成 `BetterDesktop.Shell.MenuBar.Windows`（CS0234）——需要引用 WinRT 类型时必须写 `global::Windows.…`。

## 17. 搜索结果右键菜单（统一项集 · 2026-09-14）

搜索面板（`Windows/SearchPopupWindow`）的结果右键菜单分两类处理：

- **应用类结果（`SearchResult.AppItem != null`）**：项集来自共用构建器 `packages/api/AppSource/AppEntryMenuBuilder`（与应用提取器**同一套判定**），动作走 `shell-core/Services/AppEntryActions`。项集：打开（默认加粗）→ 以管理员身份运行 → 固定到 Dock / 取消固定 → 打开所在目录 → 复制路径 → 在终端中打开 → 属性 → 卸载…（`UninstallString` 与动作同时具备才出现）。
- **设置 / 文件类结果**：保留本面板自己的通用项（打开 / 打开所在位置 / 复制路径 / 复制链接）；**文件类命中里的程序（.exe/.lnk/.url，2026-09-17）**经 `IAppSourceService.ResolveFromPath` 解析为应用条目后，在「打开」后追加「固定到 Dock / 从 Dock 移除」（复用同一 `IPinningService`，固定态判定一致）——引擎索引命中的 MAA.exe 等不再是「只能打开的文件」，可一键固定。

要点：

1. **固定项不再靠可空 `Get`**：`MenuBarPlugin.Inject` 已声明 `IPinningService`（`Inject` 是**硬依赖**——内核在依赖未满足时让插件停在 `Pending` 且不调用 `LoadAsync`）。此前用可空 `Get`，取不到就整项省略固定，菜单项随装配顺序消失（历史坑：dock 先于 context-menu 激活导致 `Get` 恒 null）。声明不引入新约束：`StartMenuPlugin.Inject` 早已声明同一服务。
2. **「打开所在目录」文案统一**：原为「打开所在位置」，现与 dock 侧及 Windows 一致。
3. 构建器**只判定项集与可用性、不执行动作**，渲染仍由本面板自理（不把自绘菜单改造成系统原生菜单——2026-09-05 拍板：自研右键仅存 dock 图标与应用提取器两处，搜索面板属「我方功能面板内部条目」）。
