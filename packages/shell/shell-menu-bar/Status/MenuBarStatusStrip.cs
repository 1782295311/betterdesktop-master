...
// 注意：本文件若在成员里写 Windows.Devices.*，"Windows" 会被 BetterDesktop.Shell.MenuBar.Windows 命名空间
// 遮蔽（CS0234），必须在文件顶部（namespace 之前）using 全局 Windows.Devices.Radios。
...
/// <summary>
/// 菜单栏蓝牙图标：开关态实时跟随系统（订阅 WinRT Radio.StateChanged）。
/// 事件三纪律（对齐 104-音量变更实时回调）：回调源由本类强引用防 GC / 回调只编队回 UI 线程 /
/// Dispose 严格反注册。WinRT 不可用时降级为静态图标。
/// </summary>
internal sealed class BluetoothIcon : ContentControl, IDisposable
{
    ...
    public BluetoothIcon()
    {
        ...
        Canvas.SetLeft(_glyph, 3.7);
        Canvas.SetTop(_glyph, 0.9);
    ...
    private async System.Threading.Tasks.Task InitializeRadioAsync()
    {
        try
        {
            var radios = await RadioInterop.GetRadiosAsync();
            var radio = radios.FirstOrDefault(r => r.Kind == RadioKind.Bluetooth);
            if (radio is null || _disposed)
            {
                return;
            }
            _radio = radio;
        ...
        {
            // WinRT 不可用（权限/服务未起）：保持静态图标（M10 降级）。
        }
    }

    private void OnRadioStateChanged(Radio sender, object args)
    {
        // 事件回调只编队回 UI 线程，不直接碰 UI（104 纪律②）。
        UiDispatch.Run(this, () =>
        {
            if (!_disposed)
            {
                ApplyState(sender.State);
            }
        });
    }

    private void ApplyState(RadioState state)
    {
        switch (state)
        {
            case RadioState.On:
            ...
            case RadioState.Disabled:
                _glyph.Opacity = 0.4; // 变暗表示无线电已关
                Visibility = Visibility.Visible;
                break;
            default:
                Visibility = Visibility.Collapsed; // Unknown：无可靠状态，隐藏避免误导
                break;
        }
    }
    ...
    public void Dispose()
    {
        ...
        {
            try { _radio.StateChanged -= OnRadioStateChanged; }
            catch { /* 已释放 */ }
            _radio = null;
        }
    }
...
public sealed class MenuBarStatusStrip : StackPanel, IDisposable
{
    ...
    private readonly BluetoothIcon _bluetoothIcon;
    ...
    public MenuBarStatusStrip(IVolumeMonitor? vol, IMicrophoneMonitor? mic, IBatteryMonitor? bat, IImeMonitor? ime, IBrightnessMonitor? brightness, INetworkMonitor? net, IMemoryMonitor? mem, ICpuMonitor? cpu, ISettingsService? settings = null)
    {
        ...
        _bluetoothIcon = new BluetoothIcon();
        base.Children.Add(CreateButton(MenuBarStatusButtonId.Bluetooth, "蓝牙", _bluetoothIcon, 11.0));
    ...
    private static void ShowDesktop()
    {
        ...
        catch
        {
            try
            {
                NativeMethods.keybd_event(91, 0, 0u, UIntPtr.Zero);
                NativeMethods.keybd_event(68, 0, 0u, UIntPtr.Zero);
                NativeMethods.keybd_event(68, 0, 2u, UIntPtr.Zero);
                NativeMethods.keybd_event(91, 0, 2u, UIntPtr.Zero);
            }
            catch
            {
            }
    ...
    public void Dispose()
    {
        ...
        _memoryIcon.Dispose();
        _fpsIcon.Dispose();
        _bluetoothIcon.Dispose();
        // B1：托盘图标订阅 NotificationArea 集合变更 + refCount ShellManager，不释放即泄漏。
