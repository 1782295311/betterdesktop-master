...
/// <summary>
/// 全局外观服务（单一外观来源）。
/// 一切外壳窗口（继承 <see cref="ShellWindow"/>）经此读取并订阅外观令牌；
/// 任何写入（主题板块控件、未来 ThemeCenter）都会触发 <see cref="Changed"/>，
...
public interface IAppearanceService
{
    ...
    /// <summary>窗口根背景画刷——基类 <see cref="ShellWindow"/> 把它同时赋给 Window.Background 与
    ...
    /// <summary>外观任意令牌变更后触发（含何种令牌变更，供订阅方按需局部重绘）。</summary>
    event EventHandler<AppearanceChangedArgs> Changed;

    /// <summary>手动触发一次全量重绘通知（如皮肤文件就绪后）。</summary>
