...
ConversionMatrix（源→目标矩阵）+ EngineRegistry（多引擎候选链解析/真实探测缓存）+ 泛化 ConversionService
驱动右键菜单的完整文件转换能力（计划：docs/plans/2026-09-03-conversion-matrix-markdown-hub.md）。
...
- EngineDownloader：按需下载五要素（多镜像/SHA256/单实例/可取消/清半包）；
  校验和未配置 = 下载禁用（无校验不下载，诚实红线）

## 安全输出（继承不重写）

同卷临时目录 .bd-convert-{pid}-{guid} → 回读验证（失败恰好重试一次）→ File.Move 原子发布 →
重名自动 (2)(3) 序号永不覆盖 → 输出≠任一输入 → 错误六分类 → convert/* 事件（含 batch-finished 计数）。
