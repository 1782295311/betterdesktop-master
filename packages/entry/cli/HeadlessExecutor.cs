...
public static class HeadlessExecutor
{
    ...
    {
        // 【2026-09-20 收敛】原先这里是 CLI 自己定位 + Process.Start 拉起桌面控制菜单进程。
        // 现在改为请 core 启动 `desktop-controls` 组件：参数（--desktop-controls）写在组件表里，
        // 不再由调用方凭记忆拼 —— 而"漏传参数会退化成拉起常驻服务"这个坑，
        // 正是把参数交给组件表管之后**结构上不可能再犯**的那一类。
        if (CoreComponents.Start(CoreComponents.DesktopControls, Diag))
        {
            return ExitCodes.Ok;
        }
        ...
        Diag("桌面控制不可用：core 不可达且宿主未运行");
        ShowError("无法打开「桌面控制」：BetterDesktop Core 未响应，请确认已安装并正在运行。");
        return ExitCodes.NeedsHost;
    }
    ...
    {
        // 【2026-09-20 收敛】不再由 CLI 定位 exe（路径候选是 core 的职责）。
        // 失败时回退拉起宿主（老部署行为，保证功能不丢）。
        if (!CoreComponents.Start(CoreComponents.Desktop, Diag))
        {
            Diag("请 core 拉起桌面服务未成功，回退拉起宿主（老部署）");
            LaunchHost();
        }
    }
    ...
    {
        // 【2026-09-20 收敛】宿主是 `shell` 组件，拉起由 core 负责（它还要判 gate 与"已在跑"）。
        if (!CoreComponents.Start(CoreComponents.Shell, Diag))
        {
            Diag("请 core 拉起宿主未成功（core 不可达），下次手动启动生效");
        }
    }
