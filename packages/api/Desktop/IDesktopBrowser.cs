...
public interface IDesktopBrowser
{
    ...
    /// <summary>在当前浏览目录新建任意格式文件（"新建{templateName}.{ext}"，重名自增），完成后刷新。
    /// content 为空 → 0 字节空文件；非空 → 写模板内容（文本/二进制/OOXML）。</summary>
    void CreateFile(string templateName, string extension, byte[]? content);
    ...
    /// <summary>设置排序键与方向并重新枚举（null 恢复智能默认；descending 默认 false=升序）。
    /// 持久化由设置层负责。</summary>
    void SetSort(string? key, bool descending = false);
