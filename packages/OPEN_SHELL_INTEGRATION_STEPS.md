...
### 目标 把 `RunningAppDetector` 从 Dock 抽出为独立服务包，Dock 改为 inject 它。Dock 行为完全不变。
...
### 目标 把 Dock 的固定/收藏能力抽成通用服务，按 zone 分区。兼容旧 `dock-pinned.json`。
...
### 目标 加程序树（层级）和变化通知事件；删除 DockAppsService 的重复扫描代码。
...
### 目标 把 DwmThumbnail 从 Dock 搬到 window-tracker，供任务栏/开始菜单未来使用。
...
### 目标 从纯 TTB 桥接改为 WPF 自绘菜单骨架，TTB 保留为回退。本步骤只做骨架 + 经典单列布局 + Win 键。
...
StartMenuPlugin 已在 Bootstrap.cs 注册（第126行），确认其 Inject 依赖在它之前注册： AppSourcePlugin → WindowTrackerPlugin → PinningPlugin → SearchPlugin → RecentPlugin → SettingsPlugin → StartMenuPlugin。
...
### 目标 AppGrabberWindow 的能力由开始菜单 + 搜索接管，删除冗余窗口。
