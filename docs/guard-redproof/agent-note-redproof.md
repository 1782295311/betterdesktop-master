...
> 取证环境 E1–E3：**N/A**（纯文档门禁，无构建产物）；E4：注入前 HEAD = `f47e7d9`；注入 commit = `2cebed4`；撤销 commit = `eee979b`

## ① 可复现的注入片段

```
2cebed4 redproof(agent-note): 注入 Status 非法的决策记录
...
撤销方式：`git revert --no-edit 2cebed4`，撤销 commit = `eee979b`。撤销后重跑（退出码 0）：
