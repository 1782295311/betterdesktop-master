...
| 项 | 值 |
|---|---|
| 当前阶段 | **P1 内核进行中**：核心契约与实现已落地（服务图 / 依赖驱动重载 / effect 托管清理 / 事件五分发）；契约测试 15 例全绿；ADR-002 已冻结内核 API |
...
| 更新时间 | 2026-08-20 |
| 下一步 | P1 剩余：kernel-loader（cordis.yml 插件树装配）与 kernel-timer（托管定时器）包 + 单测 → 覆盖棘轮与架构测试机检 → 验收后进 P2（薄宿主 + 最小外壳 + HMR） |
