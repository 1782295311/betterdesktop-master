...
| # | 项 | 为什么它独立 | 完成判据 |
|---|---|---|---|
| 1 | **假测试扫描**（是一"**类**"，不是一"**例**"） | `hotkeys.rs` 的恒真式断言（本节末尾）已证明这类问题**以绿的姿态出现** —— 恒真式断言、恒真的绕过条件、空返回的 mock、被短路掩盖的判据。**191 单测里能相信的只有读过的部分** | 扫出候选清单即可，**不追求一次扫完**：目标是把"一类"变成"可复现的检查"，哪怕只覆盖同一形态（`assert!(!(!`、`assert!(true)`、恒真空 mock） |
| 2 | **clippy 剩余 10 条警告** | 与本工作流耦合最小，纯本地 | `cargo clippy --all-targets` 的警告数下降 |
...
| **S5-4 的 8 个动作项**（暂停监护 / 系统集成×4 / 更新×2 / 恢复 / 诊断包 / 打开日志 / 关于 / 开机自启） | ✅ | ❌ **全部只在单测里绿** |
| S5-5 卸载 | ✅ | ❌ 需 B1 部署（脚本随包分发） |
| S4-5 真机验证清单 | — | ❌ |
