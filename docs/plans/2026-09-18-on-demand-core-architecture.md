...
#### 13.22.11 三处修复：菜单重复 / 静默失败 / 多余实例定性（2026-09-20）

**① 菜单重复 —— 用户判断"功能管理的设计出错了"，成立**

自绘右键菜单里「桌面控制」与「功能管理」**管同一个设置键、且语义相反**：

| | 「桌面控制」（`DesktopControlMenu.Build`）| 「功能管理」（`DesktopIconsControl`）|
|---|---|---|
| 菜单栏 | `菜单栏显隐` —— **勾 = 显示** | `隐藏菜单栏` —— **勾 = 隐藏**（`!值`）|
| Dock | `Dock 显隐` —— **勾 = 显示** | `隐藏 Dock` —— **勾 = 隐藏** |
| 任务栏 | `隐藏任务栏` —— 勾 = 已隐藏（取**实际**可见性）| `隐藏任务栏` —— 勾 = 隐藏（取**意图**键）|

⇒ 同一个开关在两处呈现相反的勾选态，点哪边都像"另一个地方变了"。

**分工按"能力是否存在" vs "部件此刻显不显示"切**：
- **功能管理** = 功能总开关（`自绘桌面` / `系统右键·格式转换`）；
- **桌面控制** = 桌面部件的即时显隐（含勾选态取真值、优先级裁决，见 `DesktopControlRules`）。

⇒ 删除「功能管理」里那三项（**不是隐藏** —— 留着只会继续漂移），并把提示改成"这里只管功能是否存在"。

**② `--toggle-key` 的静默失败（用户点名要改）**

`DesktopToggleExecutor` 末尾原本是：

```csharp
// 走到这里说明该开关既无原生效果、宿主又不在（正常不会发生：菜单已置灰）。
DesktopControlLog.Trace($"切换（免宿主）：{spec.SettingsKey}={next}（该开关无原生效果，等宿主上线应用）");
```

**原注释的前提是错的**："菜单已置灰"**只对宿主内自绘菜单成立**（`DesktopControlMenu` 按 `hostRunning` 置灰）。
但**系统右键菜单**那条路不经过它 —— explorer → `Cli.exe --menu-batch` → 拉起本进程 `--toggle-key <name>`，
**全程没有宿主在线探测这一层**，所以这些项在系统右键里照常可点，点了就走到这里。

⇒ 旧行为：只写一行日志、**以退出码 0 结束** ⇒ 用户以为生效了。
**这正是 `defensive-patterns.md` 反复批判的形态：*"将来会被应用的意图"与"已经生效"对用户不可区分*。**

修法（用户要求"报错或明确提示"）：
- 新增 `DesktopControlNotice.ShowPendingUntilHost(...)`：弹框明说**未能生效** + 已暂存哪个键值 + 主程序启动后会自动应用；
- `DesktopToggleCatalog` 的 `DesktopToggle` 加 `DisplayName`（中文名）—— 独立进程拿不到宿主内菜单那份中文标题，
  放在 catalog 才符合"单一真相源"（该文件头就是为映射漂移而设）；
- 提示失败**不影响主流程**（短命进程崩了用户只会看到"点了没反应"）。

**验证**：部署新 dll 后（`GetTypes` 确认 `DesktopControlNotice` 存在），走了一次
`DesktopControl.exe --toggle-key doubleclick` —— 此时服务在线，日志为 `已请桌面服务翻转开关：doubleclick`
（**没**走提示分支）⇒ 这正是期望行为：提示只在真"未能生效"时出现。

**③ "多余实例"的定性（纠正我上一轮的推断）**

上一轮我把它记成"core 启动窗口重试的残留"。**干净实验推翻了**：

| 步骤 | 观察 |
|---|---|
| 杀掉所有 `DesktopControl` + 重启 core | core 日志**只有一条** `started 'desktop' ... (restart #1)` |
| 30 秒后数实例 | **2 个**（14:33:28 / 14:33:30）|

⇒ **core 只拉了一次，第二个不是 core 拉的** ⇒ 来源是 `DesktopControl` **自身的启动行为**（属并行工作流）。
⇒ **core 侧的部分**（`components.json` 缺 `liveness` 导致每 3 秒重拉、`restart #2..#6` 到熔断）**已修**（§13.22.10），
本次实验证明它不再复发。剩下的那一半交回并行工作流。
