...
internal sealed class ComponentBootstrapper
{
    ...
    /// <summary>
    /// 功能件核对：目前只有"格式转换引擎树"这一项。
    /// <para>
    /// 【2026-09-18 真机：格式转换 80% 不可用 + 菜单缺一大半选项】所有转换引擎按
    /// <c>AppContext.BaseDirectory\engines\...</c> 定位（shell-convert），而模块包把 2.8GB 引擎树
    /// 单独放在 <c>06-格式转换引擎</c>（01-主程序 刻意不含它）→ 若安装时没带上，
    /// 除纯托管图片转换外全部探不到；且 <c>ConvertMenuService</c> 对不可用目标是**整项隐藏**，
    /// 所以用户看到的是"选项少了"而不只是"置灰"。
    ...
    private void VerifyFeatureFiles(Action<StepState, string, string> add)
    {
        ...
        {
            add(StepState.Ok, "格式转换引擎", "engines\\ 目录就位（格式转换 / OCR 可用）");
            return;
        }

        var source = FindEnginesSource();
        var detail = source is null
            ? "engines\\ 目录缺失：格式转换只剩纯托管图片类可用。把发布包 06-格式转换引擎 里的 engines 文件夹拷到程序目录即可"
            : $"engines\\ 目录缺失：把 {source} 里的内容复制到 {Path.Combine(ComponentPaths.BaseDir, "engines")} 即可恢复格式转换（或运行该目录下的 install-engines.ps1）";
        add(StepState.Warn, "格式转换引擎", detail);
    }

    /// <summary>在常见发布形态里找引擎树（模块包 06-格式转换引擎\engines / 发布根 engines）。</summary>
        ...
        foreach (var dir in ComponentPaths.SearchDirs)
        {
            try
            {
                ...
                foreach (var candidate in new[]
                         {
                             Path.Combine(dir, "engines"),
                             parent is null ? string.Empty : Path.Combine(parent, "06-格式转换引擎", "engines"),
                             parent is null ? string.Empty : Path.Combine(parent, "engines"),
                         })
                {
